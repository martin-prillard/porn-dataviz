...
if xhamster_tags.shape[0] >= number_of_bins:
    ...
    df_groups = pd.cut(xhamster_tags[feature], range(0, int(math.ceil((xhamster_tags[feature].max().astype(int)))), int(math.ceil((xhamster_tags[feature].max().astype(int))/nb_of_groups))))
...
if feature == 'runtime':
    feature = 'x_runtime'
file_name = "JSON_Files/Fix/" + feature + "_" + str(cluster_number) + ".json"
