...
def generate_json_from_tags(tags, feature, number_of_bins):
    ...
    if xhamster_tags.shape[0] >= number_of_bins:
        ...
        df_groups = pd.cut(xhamster_tags[feature], range(0, xhamster_tags[feature].max().astype(int), int(math.ceil((xhamster_tags[feature].max().astype(int))/nb_of_groups))))
    ...
    if feature == 'runtime':
        feature = 'x_runtime'
    file_name = "JSON_Files/" + feature + "_" + "_".join(tags) + ".json"
...
Features = ['nb_views', 'nb_comments', 'runtime']
for tags in Tags:
    for feat in Features:
        generate_json_from_tags(tags, feat, 50)
